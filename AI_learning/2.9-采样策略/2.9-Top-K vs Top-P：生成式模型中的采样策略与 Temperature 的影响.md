...
> [文章](2.9-%20深入理解%20Beam%20Search：原理%2C%20示例与代码实现.md) 中探讨了 Beam Search 和 Greedy Search，现在来聊聊 `model.generate()` 中常见的三个参数: top-k, top-p 和 temperature。

## 目录

- [什么是 Top-K 和 Top-P 采样？Temperature 如何影响生成结果？](#什么是-top-k-和-top-p-采样temperature-如何影响生成结果)
  - [目录](#目录)
