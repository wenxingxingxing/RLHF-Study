...
实现DP的一种经典编程框架叫**主从模式 (Master-Slave)**，在这个框架里，计算GPU称为Worker，梯度聚合GPU称为Server。在实际应用中，为了尽量减少通讯量，一般可选择一个Worker同时作为Server。比如可把梯度全发到GPU0上做聚合。需要再额外说明几点：
...
- 通讯开销大。计算完毕 -> 全体同步 -> 更新参数。Server需要和每一个Worker进行梯度传输。当Server和Worker不在一台机器上时，Server的带宽将会成为整个系统的计算效率瓶颈。


## 二、分布式数据并行(DDP)

DDP：通信模式为集合通信(AllReduce)模式，采用了梯度分桶（Gradient Bucketing）技术。DDP 将参数划分为多个桶（Buckets）。当某一层反向传播计算完成时，该桶的梯度同步就开始了。这意味着，在计算后续层梯度的同时，前几层的梯度已经在进行网络同步，实现了计算与通信的并行。
...
上图中，ψ表示模型参数量，K表示优化器状态的内存倍数，以bf16混合精度训练，AdamW优化器为例，在训练过程中，模型参数和梯度的数据类型为bf16，优化器状态的数据类型为FP32。并行方式为DDP时，占用的静态显存为(2+2+12)ψ。
